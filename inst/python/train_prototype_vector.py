...
def train_prototype_hypersphere(dat_train,pretrained_model,sen_len,pretrained_model_path,save_checkpoint_path,batch_size = 1,train_K = 8,train_Q = 8, train_episodes = 300,val_episodes = 30,val_steps = 10,reproduce = False,set_seed = 42,with_gpu = True):
    ...
    with torch.no_grad():
        for j in tqdm(range(0,max(dat_train.region.value_counts()))):
            ...
            for c in list(set(dat_train.region)):
                support.append([])
                support_mask.append([])
                support_tls, support_tls_mask = tokenizer(ref_all[ref_all.region==c].iloc[j].sentence)
                support[c] = np.array(support_tls)
                support_mask[c] = np.array(support_tls_mask)
    ...
    if len(list(set(dat_train.region)))==2:
        ...
        region_similarity.columns = ["Non-TLS Cosine similarity","TLS Cosine similarity"]
        ...
        region_similarity.columns = ['Cosine_similarity ' + str(i) for i in range(0,N)]
        dat_train = pd.concat([dat_train,region_distance],axis=1)
        dat_train = pd.concat([dat_train,region_similarity],axis=1)
        dat_train["pred_label"] = predict_label.cpu().tolist()
        #dat_train["relative_distance"] = dat_train["TLS Distance"] - dat_train["Non-TLS Distance"]
