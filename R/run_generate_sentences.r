...
generate_sentences <- function(seu_obj,sen_len=260,region_info = NULL,save_inseu=TRUE,save_path=NULL,genes_representor=NULL,data_type="sc_st",envir_path){

    reticulate::use_condaenv(envir_path, required = TRUE)
    reticulate::source_python(system.file("python", "GPU_sort_index_makesentences.py", package = "Structformer"))

    if(data_type == "sc_st"){

        sc_dat_inputdf <- as.data.frame(seu_obj@assays[[1]]@counts)
        
        if(!is.null(genes_representor)){
